...
{
    public class CommandsBuilderOld(IServiceCollection services)
    {
        ...
        {
            var code = ConvertAndValidateCommandCode(inputCommandCode);
            var data = CreateEntry(code, CommandType.Stateless, availableChatTypes);
            ...
            Func<IServiceProvider, object?, TCommand>? factory) where TCommand : class, ICommand
        {
            ...
            services.AddKeyedScoped<ICommandProxy, CommandExecutor>(
                commandCode,
                (services, key) =>
                {
        ...
        internal void Build()
        {
            ...
            {
                var descriptor = new CommandDescriptor(command.Code, command.Help, command.Type, command.AvailableChatTypes);
